(function (global, factory) {
    ...
    class Contract {
      ...
      test(func_name, amount_to_send, ...func_params) {
        ...
        const copy = ArrayBuffer.transfer(this.vm.instance.exports.memory.buffer, this.vm.instance.exports.memory.buffer.byteLength); // Call the function.
      ...
      parseFunctionParams(...params) {
        ...
        params.forEach(param => {
          switch (param.type) {
            ...
            case "raw":
              if (JSBI.equal(typeof param.value, "string")) {
      ...
      async fetchAndPopulateMemoryPages() {
        if (JSBI.equal(this.vm, undefined)) throw new Error("init() needs to be called before calling fetchAndPopulateMemoryPages()");
    ...
    class Wavelet {
      ...
      async getNodeInfo(opts) {
        return (await axios.get(`${this.host}/ledger`, { ...this.opts,
          ...opts
        })).data;
      }
      /**
      ...
      async getTransaction(id, opts = {}) {
        return (await axios.get(`${this.host}/tx/${id}`, { ...this.opts,
          ...opts
        })).data;
      }
      /**
      ...
      async getAccount(id, opts = {}) {
        return (await axios.get(`${this.host}/accounts/${id}`, { ...this.opts,
          ...opts
        })).data;
      }
      /**
      ...
      async getCode(id, opts = {}) {
        return new Uint8Array((await axios.get(`${this.host}/contract/${id}`, { ...this.opts,
          ...opts,
      ...
      async getMemoryPages(id, num_mem_pages, opts = {}) {
        ...
        for (let idx = 0; idx < num_mem_pages; _x3 = idx, idx = JSBI.add(idx, JSBI.BigInt(1)), _x3) {
          var _x3;

          try {
            const res = await axios.get(`${this.host}/contract/${id}/page/${idx}`, { ...this.opts,
              ...opts,
              responseType: 'arraybuffer',
              responseEncoding: 'binary'
            });
      ...
      async sendTransaction(wallet, tag, payload, opts = {}) {
        ...
        return (await axios.post(`${this.host}/tx/send`, req, { ...this.opts,
          ...opts
        })).data;
      }
      /**
      ...
      pollAccounts(callbacks = {}, opts = {}) {
        let params = {};
        if (opts && opts.id && JSBI.equal(typeof opts.id, "string") && opts.id.length === 64) params.id = opts.id;
      ...
      pollTransactions(callbacks = {}, opts = {}) {
        let params = {};
        if (opts && opts.id && JSBI.equal(typeof opts.id, "string") && opts.id.length === 64) params.id = opts.id;
        if (opts && opts.tag && JSBI.equal(typeof opts.tag, "number")) params.tag = opts.tag;
        if (opts && opts.sender && JSBI.equal(typeof opts.sender, "string") && opts.sender.length === 64) params.sender = opts.sender;
        if (opts && opts.creator && JSBI.equal(typeof opts.creator, "string") && opts.creator.length === 64) params.creator = opts.creator;
          ...
          case TAG_BATCH:
            {
              ...
              for (let i = 0, offset = 1; JSBI.lessThan(i, len); _x4 = i, i = JSBI.add(i, JSBI.BigInt(1)), _x4) {
                var _x4;
                ...
                offset = JSBI.add(offset, payloadLen);
